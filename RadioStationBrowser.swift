                                        ...
                                        if zone.isPlaying {
                                            let context: String = {
                                                if !zone.currentTrack.isEmpty {
                                                    return zone.stationName.isEmpty ? zone.currentTrack : "\(zone.stationName) · \(zone.currentTrack)"
                                                } else if !zone.stationName.isEmpty { return zone.stationName }
                                                return "Playing"
                                            }()
                                            Text(context).font(.system(size: 12)).foregroundColor(.sHighlight).lineLimit(1)
                                        } else if !zone.stationName.isEmpty {
                                            Text(zone.stationName).font(.system(size: 12)).foregroundColor(.sTextMuted).lineLimit(1)
                                        } else {
                                            Text("Idle").font(.system(size: 12)).foregroundColor(.sTextMuted)
                                        }
                                    }
                                    Spacer()
                                    Image(systemName: "chevron.right").font(.system(size: 12)).foregroundColor(.sTextMuted)
                                }
